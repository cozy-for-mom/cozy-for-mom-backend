...
//        FindGrowthDiaryRecord findGrowthDiaryRecord = babyProfileRepository.searchGrowthRecord(userId, date);
        BabyProfile findBabyProfile = babyProfileRepository.findByBabyProfileId(babyProfileId).orElseThrow(() -> new IllegalArgumentException("존재하지 않는 아이입니다."));
        GrowthDiary findGrowthDiary = growthDiaryRepository.findByBabyProfileAndRecordAt(findBabyProfile, date).orElseThrow(() -> new IllegalArgumentException("존재하지 않는 성장일기입니다."));
        FindGrowthDiaryRecord findGrowthDiaryRecord = new FindGrowthDiaryRecord(findBabyProfile.getBabyProfileId(), findGrowthDiary, findBabyProfile.getBabyList());
